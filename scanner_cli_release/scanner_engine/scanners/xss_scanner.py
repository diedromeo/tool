...
class XSSScanner:
    ...
    def _add_vulnerability(self, v_type, severity, url, param, payload, evidence, response_obj=None):
        with self.lock:
            ...
            if response_obj:
                req_str = format_http_request(response_obj.request)
                res_str = format_http_response(response_obj)
    ...
    def _check_reflection(self, response, payload, url, param, v_prefix):
        ...
        try:
            ...
            if re.search(r'<!--.*?'+escaped_payload+r'.*?-->', html, re.DOTALL):
                return False # FP: Inside comment
            ...
            if dangerous_context:
                return False # FP: Inside non-executable tag
        except: pass
